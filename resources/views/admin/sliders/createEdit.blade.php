        ...
        <div class="col-md-12">
            <div class="portlet light bordered">
                <div class="portlet-body form">
                    ...
                    @if(isset($data->item))
                    {!! Form::model($data->item, ['route' => ['admin.Slider.update', $data->item->id], 'method' => 'post','files' => true]) !!}
                    {{ method_field('PATCH')}}
                    @else
                    ...
                    <div class="form-body">
                        ...
                        <div class="form-group col-sm-12 row">
                            @foreach($locales as $locale)
                               <div class="form-group col-sm-6">
                                    {!! Form::label('guiding_title1_'.$locale->code, $locale->name ) !!}
                                    {!! Form::text('guiding_title1_'.$locale->code, old('guiding_title1_'.$locale->code, 
                                    isset($data->item)?$data->item->translate($locale->code)->guiding_title1:'' ), ['class' => 'form-control', 'required' => 'required', 'maxlength' => 40 ]) !!}
                                </div>
                            @endforeach 
                        </div>
                        ...
                        <div class="form-group col-sm-12 row">
                            @foreach($locales as $locale)
                               <div class="form-group col-sm-6">
                                    {!! Form::label('guiding_title2_'.$locale->code, $locale->name ) !!}
                                    {!! Form::text('guiding_title2_'.$locale->code, old('guiding_title2_'.$locale->code, 
                                    isset($data->item)?$data->item->translate($locale->code)->guiding_title2:'' ), ['class' => 'form-control', 'required' => 'required', 'maxlength' => 40 ]) !!}
                                </div>
                            @endforeach 
                        </div>
                        ...
                        <div class="form-group col-sm-12 row">
                            @foreach($locales as $locale)
                               <div class="form-group col-sm-6">
                                    ...
                                    {!! Form::text('title_'.$locale->code, old('title_'.$locale->code, 
                                    isset($data->item)?$data->item->translate($locale->code)->title:'' ), ['class' => 'form-control', 'required' => 'required', 'maxlength' => 100 ]) !!}
                                </div>
                            @endforeach 
                        </div>
                        ...
                        <div class="form-group col-sm-12">
                            ...
                            <a href="{{ route('admin.Slider.index') }}" class="btn btn-warning">{{ __('translate.cancel') }}</a>
                        </div>
