        ...
        <div class="col-md-12">
            <div class="portlet light bordered">
                <div class="portlet-body form">
                    ...
                    @if(isset($data->item))
                    {!! Form::model($data->item, ['route' => ['admin.Brand.update', $data->item->id], 'method' => 'post','files' => true]) !!}
                    {{ method_field('PATCH')}}
                    @else
                    ...
                    <div class="form-body">
                        ...
                        <div class="form-group col-sm-12" style="background-color:#ccc; padding: 10px; margin-bottom:5px;">
                            {{ __('translate.name') }}
                        </div>
                        ...
                        <div class="form-group col-sm-12 row">
                            @foreach($locales as $locale)
                               <div class="form-group col-sm-6">
                                    ...
                                    {!! Form::text('name_'.$locale->code, old('name_'.$locale->code, isset($data->item)?$data->item->translate($locale->code)->name:'' ), ['class' => 'form-control', 'required' => true]) !!}
                                </div>
                            @endforeach 
                        </div>
                        ...
                        <div class="form-group col-sm-6">
                            <div class="fileinput-new thumbnail" onclick="document.getElementById('edit_image').click()" style="cursor:pointer">
                                <img src="{{ @$data->item->image? url($data->item->image) : url('uploads/ChoosePhoto.png') }}" id="editImage" style="max-width: 200px;">
                            </div>
                        ...
                        <div class="form-group col-sm-12">
                            ...
                            <a href="{{ route('admin.Brand.index') }}" class="btn btn-warning">{{ __('translate.cancel') }}</a>
                        </div>
