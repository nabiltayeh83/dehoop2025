...
use Illuminate\Support\Facades\Cache;
    ...
    function calculateOrderCost($car_type_id, $lat1, $lon1, $lat2, $lon2){
        $cost = null;
        ...
        if($car_type){
            if($distance <= $car_type->minimum_distance){
                $cost = $car_type->mandatory_charge;
            }
            else{
                $cost = $car_type->mandatory_charge + (($distance - $car_type->minimum_distance) * $car_type->price_for_each_km);
            }
        }
        return round($cost);
    }
