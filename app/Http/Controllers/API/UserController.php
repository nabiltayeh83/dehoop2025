...
class UserController extends Controller
{
    ...
    public function signUpUsers(Request $request)
    {
        ...
        if($checkUser){
            if($checkUser->status == 'not_active'){
                return response()->json(['status' => false, 'code' => 200, 'message' => 'Your account is not active'  ]);   
            }
        }
        
        Code::where('mobile', $request->mobile)->delete();
        
        $code = 1111;
        $conf = new Code();
        $conf->code = $code;
        $conf->mobile = $request->mobile;
        $conf->save();
            
        return response()->json(['status' => true, 'code' => 200, 'message' => 'Activation code sent to your mobile', 'data' => $conf ]);
    ...
    public function loginForUsers(Request $request)
    {
        ...
        if($checkUser->status == 'not_active'){
            return response()->json(['status' => false, 'code' => 201, 'message' => 'Your account not active' ]);
        }
    ...
    public function startFundRaise(Request $request)
    {
        ...
        if($user){
            ...
            User::where('mobile', $request->mobile)->update(['status' => 'active']);
    ...
    public function checkCode(Request $request)
    {
        ...
        $checkCode = Code::where(['mobile' => $request->mobile, 'code' => $request->code, 'used' => 0])->orderBy('id', 'desc')->first();
        ...
        return response()->json(['status' => true, 'code' => 200, 'message' => 'Account created successfully', 'data' => $checkUser ]);
    ...
    public function reSendCode(Request $request)
    {
        ...
        if($checkUser){
            ...
            $msg = 'Validation Code is ' . $code;
            return response()->json(['status' => true, 'code' => 200, 'message' => 'Activation code sent to your mobile', 'data' => $checkUser]);
        }    
